...
**com** $log(K_{n}(\eta))=\frac{n}{2}log(\frac{c(\eta)}{\Pi})+log\Gamma(\frac{1+{n}\eta}{2\eta}), \delta=(\boldsymbol{Y} - \chi\beta)^T\Sigma^-1(\boldsymbol{Y} - \chi\beta) **e** c(\eta)=(\eta)/(1 - 2(\eta)$
